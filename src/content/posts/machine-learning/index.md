...
and the model outputs a complete word "hello". We call inputs like "hell" a __sequence__.

How do we train such model? One approach is to have __one function invoked 4 times__, with each time taking a single
character as input and calculates an output:

![](./img/rnn-4-black-boxes.png)

:::tip[Input for the function is actually a one-hot encoded vector representing a single character]
...
same input, outputs a different character of 'o'? This suggests that we should take the __history__ into
...
function gets invoked __repeatedly__ for each element of the sequence. In the case of our character-level language model,
element is a character such as "e" and sequence is a string like "hell":

:::caution

The diagram below is not multiple functions chained together, but a single function being __repeatedly__ invoked
...
A Python implementation of the RNN discussed in this article can be found [here](https://github.com/QubitPi/RNN)

::github{repo="QubitPi/RNN"}
